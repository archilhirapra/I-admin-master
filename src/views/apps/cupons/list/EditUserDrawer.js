...
const SidebarAddUser = props => {
  ...
  useEffect(() => {
    ...
    setValue('terms', editRow?.terms)
    setValue('minimumAmount', editRow?.minimumAmount)


    setValue('isVisible', editRow?.isVisible)
    setValue('percentage', editRow?.percentage)
    setValue('isOnce', editRow?.isOnce)


    setValue('isExpired', editRow?.isExpired)
    setValue('isExist', editRow?.isExist)
    setValue('isNewOnly', editRow?.isNewOnly)
    setValue('isSpecial', editRow?.isSpecial)

    if (editRow?.start != undefined) {
      let [startDateComponents, timeComponents] = editRow.start.split(' ');
      let [startMonth, startDay, startYear] = startDateComponents.split('-');
      let startFinalDate = new Date(+startYear, startMonth - 1, +startDay);
      setValue('start', startFinalDate)
    }
    if (editRow?.end != undefined) {
      let [endDateComponents, timeComponents] = editRow.end.split(' ');
      let [endMonth, endtDay, endYear] = endDateComponents.split('-');
      let endFinalDate = new Date(+endYear, endMonth - 1, +endtDay);
      setValue('end', endFinalDate)
    }
  }, [editRow])
