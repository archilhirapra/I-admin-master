...
const EditTimeSlots = props => {
  ...
  // ** Props
  const {open, toggle, editRow, setEditRow} = props

  useEffect(() => {


    if (editRow?.date != undefined) {
      setValue('date', new Date(editRow?.date))
    }

    setValue('', new Date(editRow?.date))

    let slots = [];

    const found = editRow?.timeSlots.find(
      obj => {
        return obj.time === '09:00-11:00'
      }
    )

    setAllSwitchValues(
      {
        fullHoliday: editRow?.isFullHoliday,
        halfHoliday: editRow?.isHalf,

        // time05: (editRow?.timeSlots.find(obj => { return obj.time === '09:00-11:00' })?.isActive),

        time5ato6a: (editRow?.timeSlots.find(obj => {
          return obj.time === '05:00-06:00'
        })?.isActive),
        time6ato7a: (editRow?.timeSlots.find(obj => {
          return obj.time === '06:00-07:00'
        })?.isActive),
        time7ato8a: (editRow?.timeSlots.find(obj => {
          return obj.time === '07:00-08:00'
        })?.isActive),
        time8ato9a: (editRow?.timeSlots.find(obj => {
          return obj.time === '08:00-09:00'
        })?.isActive),
        time9ato10a: (editRow?.timeSlots.find(obj => {
          return obj.time === '09:00-10:00'
        })?.isActive),
        time10ato11a: (editRow?.timeSlots.find(obj => {
          return obj.time === '10:00-11:00'
        })?.isActive),
        time11ato12p: (editRow?.timeSlots.find(obj => {
          return obj.time === '11:00-12:00'
        })?.isActive),
        time12pto1p: (editRow?.timeSlots.find(obj => {
          return obj.time === '12:00-13:00'
        })?.isActive),
        time1pto2p: (editRow?.timeSlots.find(obj => {
          return obj.time === '13:00-14:00'
        })?.isActive),
        time2pto3p: (editRow?.timeSlots.find(obj => {
          return obj.time === '14:00-15:00'
        })?.isActive),
        time3pto4p: (editRow?.timeSlots.find(obj => {
          return obj.time === '15:00-16:00'
        })?.isActive),
        time4pto5p: (editRow?.timeSlots.find(obj => {
          return obj.time === '16:00-17:00'
        })?.isActive),
        time5pto6p: (editRow?.timeSlots.find(obj => {
          return obj.time === '17:00-18:00'
        })?.isActive),
        time6pto7p: (editRow?.timeSlots.find(obj => {
          return obj.time === '18:00-19:00'
        })?.isActive),
        time7pto8p: (editRow?.timeSlots.find(obj => {
          return obj.time === '19:00-20:00'
        })?.isActive),


        // time9t11: (editRow?.timeSlots.find(obj => { return obj.time === '09:00-11:00' })?.isActive),
        // time11t1: (editRow?.timeSlots.find(obj => { return obj.time === '11:00-13:00' })?.isActive),
        // time1t3: (editRow?.timeSlots.find(obj => { return obj.time === '13:00-15:00' })?.isActive),
        // time3t5: (editRow?.timeSlots.find(obj => { return obj.time === '15:00-17:00' })?.isActive),
        // time5t7: (editRow?.timeSlots.find(obj => { return obj.time === '17:00-19:00' })?.isActive),
        // time7t9: (editRow?.timeSlots.find(obj => { return obj.time === '19:00-21:00' })?.isActive),
      }
    )
  ...
  const continueEditing = () => {
    ...
    dispatch(addHoliday(finaldata)).then((data) => {
      ...
      if (data?.payload?.status == "success") {
        ...
        setDialogMessage('Holiday successfully edited')
        handleClickOpenDialog()
      } else {
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h6'>Edit Time Slot</Typography>
