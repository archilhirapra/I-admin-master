...
// import { addCategory, getCategoryDetails, editCategory } from 'src/store/apps/services/categories'
...
const SidebarAddUser = props => {
  ...
  useEffect(() => {
    ...
    console.log(editRow?.birthDate)
    if (editRow?.birthDate != undefined) {
      // console.log(editRow?.birthDate)
      // console.log(Date.parse(editRow?.birthDate))
      setValue('dob', new Date('1/08/2022'))
    }

    setGenderChecked(editRow?.gender)
    // setValue('gender', editRow?.gender)
    setValue('gender', 'Male')
    setValue('role', editRow?.role)

    setPhoneInput(formatPhoneNumber(editRow?.mobileNo))
    setPhoneInputnoFormat(editRow?.mobileNo)
    // setValue('description', editRow?.description)
    // console.log(editRow?.isVisible)
    // SetIsVisibleSitch(editRow?.isVisible)
    // SetIsSubscriptionSitch(editRow?.isSubscription)
  ...
  const continueEditing = () => {
    handleCloseDialog()
    if (phoneInputnoFormat != '') {
      let data = filledFormData
      setLoading(true)
      // let date = new Date(data.dob).toLocaleDateString('en-US');
      ...
      dispatch(addAdmin({ name, email, mobileNo: phoneInputnoFormat, dob: date, gender, role })).then(data => {
        console.log(data)
        setLoading(false)
  ...
  const [phoneInputnoFormat, setPhoneInputnoFormat] = useState('')
  const handlePhoneInput = e => {
    setPhoneInputnoFormat(e.target.value)
