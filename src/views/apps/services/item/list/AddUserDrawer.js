...
const SidebarAddUser = props => {
  ...
  const onSubmit = data => {
    ...
    setLoading(true);
    const {name, description, isVisible, isSubscription} = data
    dispatch(addCategory({
      name,
      description,
      isVisible: isVisibleSitch,
      isSubscription: isSubscriptionSitch,
      image: files[0]
    })).then((data) => {
      console.log(data);
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Box sx={{mb: 6}}>
            <FormControlLabel label='Status'
                              control={<Switch checked={isVisibleSitch} onChange={handleVisibilitySwitch}/>}/>
          </Box>
          <Box sx={{mb: 6}}>
            {/* <FormControlLabel label='Subscription Allowed' control={<Switch defaultChecked />} /> */}
            <FormControlLabel label='Subscription Allowed'
                              control={<Switch checked={isSubscriptionSitch} onChange={handleSubscriptionSwitch}/>}/>
          </Box>
