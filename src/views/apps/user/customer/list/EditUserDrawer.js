...
const EditAdminDrawer = props => {
  ...
  useEffect(() => {
    ...
    if (editRow?.birthDate != undefined) {
      let [startDateComponents, timeComponents] = editRow.birthDate.split(' ')
  ...
  const continueEditing = () => {
    ...
    const { name, email, mobileNo, dob, gender, role, aMobileNo, fatherName, blood, status } = data
            ...
            <Grid item xs={12}>
              <FormControl fullWidth sx={{ mb: 6 }}>
                <Controller
                  ...
                  rules={{ required: true }}
                  render={({ field: { value, onChange } }) => (
                    <FormControlLabel label='Status' control={<Switch checked={value} onChange={onChange} />} />
                  )}
                />
              </FormControl>
            </Grid>
