...
const SidebarAddUser = props => {
  ...
  const continueEditing = () => {
    ...
    const { name, email, mobileNo, dob, gender, status } = data
    ...
    dispatch(addCustomer({ name, email, mobileNo: phone, dob: date, gender, status })).then(data => {
      setLoading(false)
      ...
      <Box sx={{ p: 5 }}>
        <form onSubmit={handleSubmit(onSubmit)}>
          <Grid container spacing={5}>
            ...
            <Grid item xs={12}>
              <FormControl fullWidth>
                <InputLabel id='status' error={Boolean(errors.status)} htmlFor='status'>
                  Status
                </InputLabel>
                <Controller
                  name='status'
